"""
whisplay-daemon compatibility layer.

When the daemon socket is available, the app renders into the daemon-provided
framebuffer and uses daemon button / LED / backlight APIs instead of touching
...
DEFAULT_APP_ID = "whisplay-xiaozhi"
DEFAULT_APP_DISPLAY_NAME = "xiaozhi"
DEFAULT_APP_ICON = "AI"
...
def create_whisplay_hardware():
    ...
    if daemon.ping():
        try:
            ...
            log.warning("daemon detected but adaptation failed, fallback to direct hardware: %s", e)
    # Import direct-hardware backend lazily so daemon mode does not load SPI/GPIO
