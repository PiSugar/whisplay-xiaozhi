...
case "$(uname -s)-$(uname -m)" in
    Darwin-*)
        ARCH_DIR="darwin-$(uname -m)"
        LIB_NAME="lib_watercolor_rust.dylib"
        ;;
    Linux-*)
        LIB_NAME="lib_watercolor_rust.so"
        case "$(uname -m)" in
...
if ! command -v cargo >/dev/null 2>&1; then
    echo "cargo is required to build the Rust display renderers" >&2
    exit 1
fi

cargo build --manifest-path "$CRATE_DIR/Cargo.toml" --release
mkdir -p "$PREBUILT_DIR"
cp "$CRATE_DIR/target/release/$LIB_NAME" "$PREBUILT_OUTPUT"
