...
# Raspberry Pi camera MCP tool
CAMERA_TOOL_ENABLED = _bool("XIAOZHI_CAMERA_TOOL_ENABLED", "false")
CAMERA_COMMAND = _get("XIAOZHI_CAMERA_COMMAND")
CAMERA_VIEWFINDER_COMMAND = _get("XIAOZHI_CAMERA_VIEWFINDER_COMMAND")
CAMERA_INDEX = max(0, _int("XIAOZHI_CAMERA_INDEX", "0"))
CAMERA_WIDTH = max(160, min(1280, _int("XIAOZHI_CAMERA_WIDTH", "1280")))
CAMERA_HEIGHT = max(120, min(960, _int("XIAOZHI_CAMERA_HEIGHT", "960")))
CAMERA_QUALITY = max(30, min(95, _int("XIAOZHI_CAMERA_QUALITY", "90")))
CAMERA_WARMUP_MS = max(0, min(5000, _int("XIAOZHI_CAMERA_WARMUP_MS", "2000")))
CAMERA_TIMEOUT_SEC = max(2.0, _float("XIAOZHI_CAMERA_TIMEOUT_SEC", "15"))
CAMERA_VISION_TIMEOUT_SEC = max(
    5.0, min(180.0, _float("XIAOZHI_CAMERA_VISION_TIMEOUT_SEC", "60"))
)
CAMERA_VIEWFINDER_WIDTH = max(
    320, min(1280, _int("XIAOZHI_CAMERA_VIEWFINDER_WIDTH", "640"))
)
CAMERA_VIEWFINDER_HEIGHT = max(
    240, min(960, _int("XIAOZHI_CAMERA_VIEWFINDER_HEIGHT", "480"))
)
CAMERA_VIEWFINDER_FPS = max(
    1, min(12, _int("XIAOZHI_CAMERA_VIEWFINDER_FPS", "5"))
)
CAMERA_VIEWFINDER_QUALITY = max(
    40, min(90, _int("XIAOZHI_CAMERA_VIEWFINDER_QUALITY", "75"))
)
CAMERA_VIEWFINDER_READY_TIMEOUT_SEC = max(
    3.0, min(20.0, _float("XIAOZHI_CAMERA_VIEWFINDER_READY_TIMEOUT_SEC", "10"))
)
CAMERA_MAX_FRAME_BYTES = max(
    256 * 1024, min(5 * 1024 * 1024, _int("XIAOZHI_CAMERA_MAX_FRAME_BYTES", str(2 * 1024 * 1024)))
)
CAMERA_OUTPUT_DIR = _get("XIAOZHI_CAMERA_OUTPUT_DIR", "data/camera")
CAMERA_KEEP_CAPTURES = max(1, _int("XIAOZHI_CAMERA_KEEP_CAPTURES", "10"))
CAMERA_AUTOFOCUS = _bool("XIAOZHI_CAMERA_AUTOFOCUS", "true")
CAMERA_AUTOFOCUS_RANGE = _get("XIAOZHI_CAMERA_AUTOFOCUS_RANGE", "full")
if CAMERA_AUTOFOCUS_RANGE not in ("normal", "macro", "full"):
    CAMERA_AUTOFOCUS_RANGE = "full"
CAMERA_AUTOFOCUS_SPEED = _get("XIAOZHI_CAMERA_AUTOFOCUS_SPEED", "fast")
if CAMERA_AUTOFOCUS_SPEED not in ("normal", "fast"):
    CAMERA_AUTOFOCUS_SPEED = "fast"
CAMERA_PREVIEW_SECONDS = max(
    0.5, min(30.0, _float("XIAOZHI_CAMERA_PREVIEW_SECONDS", "2"))
)
CAMERA_USER_PHOTO_PREVIEW_SECONDS = max(
    0.5, min(30.0, _float("XIAOZHI_CAMERA_USER_PHOTO_PREVIEW_SECONDS", "2"))
)
CAMERA_DOUBLE_CLICK_SECONDS = max(
    0.20, min(0.80, _float("XIAOZHI_CAMERA_DOUBLE_CLICK_SECONDS", "0.38"))
)
